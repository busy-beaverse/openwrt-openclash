...
3. 在openclash的github界面下载最新的ipk文件，自行判断是nftable/iptables(问ai)，复制对应的安装脚本到一个sh文件（后缀名为sh），sh和ipk文件上传到路由器，通过ssh登陆到路由器输入opkg install bash，安装bash。
4. 输入vi [这里填sh文件路径]（vim编辑器自行搜索用法），把第二行末尾如果有特殊符号删掉，第三行删掉，把ipk文件改名为“openclash.ipk”，然后保存退出编辑器。
5. 输入bash [这里填sh文件路径]，回车，第一遍应该会显示dnsmasq-full安装失败，输入opkg remove dnsmasq，把之前的删掉，然后再输入bash [省略]。
6. 等待几分钟之后路由器重启，openclash应该安装好了。现在用浏览器输入路由器地址（一般是192.168.1.1）访问管理页面，在服务一栏下面点开openclash。
